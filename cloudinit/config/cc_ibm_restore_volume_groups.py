...
def handle(name, _cfg, _cloud, log, _args):
    ...
    for physical_volume_id, volume_group_name in pvid_to_vg_map.iteritems():
    ...
    expected_volume_groups = set(vg for vg in pvid_to_vg_map.values())
...
def get_physical_volume_from_lspv(log, physical_volume_id):
    ...
    try:
        env = os.environ
        env['LANG'] = 'C'
        lspv_out = util.subp([LSPV], env=env)[0].strip()
...
def set_volume_group(log, hdisk, volume_group_name):
    ...
    '''
    try:
        out = util.subp([IMPORTVG, '-y', volume_group_name, hdisk])[0]
        log.debug(out)
    except util.ProcessExecutionError:
...
def get_existing_volume_groups(log):
    ...
    try:
        env = os.environ
        env['LANG'] = 'C'
        lsvg_out = util.subp([LSVG], env=env)[0].strip()
