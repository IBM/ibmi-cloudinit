...
class DataSource(object):
    ...
    def get_instance_id(self):
        if not self.metadata or 'instance-id' not in self.metadata:
            ...
            LOG.info("No instance id found in self.metadata")
            return "iid-datasource"
        LOG.info("instance id found in self.metadata, it is %s", str(self.metadata['instance-id']))
...
def find_source(sys_cfg, distro, paths, ds_deps, cfg_list, pkg_list):
    LOG.debug("in find_source of _init_py of sources package, sys_cfg is %s", sys_cfg)
    LOG.debug("in find_source of _init_py of sources package, distro is %s", distro)
    LOG.debug("in find_source of _init_py of sources package, path is  %s", paths)
    LOG.debug("in find_source of _init_py of sources package, ds_deps is  %s", ds_deps)
    LOG.debug("in find_source of _init_py of sources package, cfg_list is  %s", cfg_list)
    LOG.debug("in find_source of _init_py of sources package, pkg_list is  %s", pkg_list)
    ...
    for cls in ds_list:
        try:
            ...
            LOG.debug("sys_cfg is %s", sys_cfg)
            LOG.debug("distro is %s", distro)
            LOG.debug("path is  %s", paths)
