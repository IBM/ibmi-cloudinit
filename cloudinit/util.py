        ...
        if os.path.isfile(exe_path) and os.access(exe_path, os.X_OK):
            ...
            try:
                for line in fh.readlines():
                    LOG.debug("[script content]: %s", line)
                subp(prefix + [exe_path], capture=True)
...
def dos2unix(contents):
    ...
    pos = contents_str.find('\n')

    if pos <= 0 or contents_str[pos - 1] != '\r':
        return contents_str
    return contents_str.replace('\r\n', '\n')
...
def pipe_in_out(in_fh, out_fh, chunk_size=1024, chunk_cb=None):
    ...
    while True:
        ...
        if not data:
            ...
            out_fh.write(bytes.decode(data))
